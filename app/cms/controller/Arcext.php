...
class Arcext extends CmsBase
{
     ...
     * Author: kfrs <goodkfrs@QQ.com> created by at 2020/7/1 0001
     */
    public function show_item_json(){
        return $this->logicArcext->getArcextExtList($this->param);
    }

    /**
     * 编辑
     */
    public function ext_add()
    {
        IS_POST && $this->jump($this->logicArcext->arcextExtAdd($this->param));

        $this->common_data();
//        $info = $this->logicArcext->getArcextExtInfo( $this->param);
//        $this->assign('info', $info);

        return $this->fetch('ext_add');
    }

    /**
     * 编辑
     */
    public function ext_edit()
    {
        IS_POST && $this->jump($this->logicArcext->arcextExtEdit($this->param));

        $info = $this->logicArcext->getArcextExtInfo( $this->param);

        $this->assign('info', $info);

        return $this->fetch('ext_edit');
    }
    ...
    {
        $this->jump($this->logicArcext->arcextExtDel($this->param));
    }
