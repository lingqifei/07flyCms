...
class TagList extends Base
{
	 ...
	 */
	public function getList($param = array(), $pagesize = '10', $orderby = '', $orderway = '', $thumb = '')
	{
		$result = false;
		...
		if (!empty($param['channelid'])) {
			...
			if (!preg_match('/^\d+([\d\,]*)$/i', $param['channelid'])) {
				echo '标签list报错：channelid属性值语法错误，请正确填写栏目ID。';
				return false;
			}
		...
		if (!empty($param['cityid'])) {
			...
			$randMap['a.sys_area_id'] = ['in', $param['cityid']];
		}

		if (!empty($param['typeid'])) {
			$where['a.type_id'] = ['in', $param['typeid']];

			$randMap['a.type_id'] = ['in', $param['typeid']];
		}
		...
		if (!empty($param['flag'])) {
			...
			$randMap['a.flag'] = ['exp', Db::raw("REGEXP '(^|,)($reg_txt)(,|$)'")];
		}
		// $param = input('param.');
		...
		//主表字段过滤,针对参数筛选
		if (!empty($param['mainfields'])) {
			$mainfieldsArr=str2arr($param['mainfields']);
			foreach ($mainfieldsArr as $mainfield){
				$mainfieldVal=input("param.{$mainfield}/s", '');
				if(!empty($mainfieldVal)){
					$where["a.".$mainfield] = ['=', $mainfieldVal];
				}
			}
		}
		...
		if (!empty($param['channelexttable'])) {
			...
			if (!empty($param['addfields'])) {
				$addfieldsArr=str2arr($param['addfields']);
				foreach ($addfieldsArr as $addfield){
					$addfieldVal=input("param.{$addfield}/s", '');
					if(!empty($addfieldVal)){
						$where["e.".$addfield] = ['=', $addfieldVal];
					}
				}
			}
