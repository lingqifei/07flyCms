...
class Template
{
    ...
    public function parse(&$content)
    {
        ...
        // 替换literal标签内容
        $this->parseFlyLiteral($content,true);
        // 还原被替换的Literal标签
     ...
     * @return void
     */
    private function parseFlyInclude(&$content)
    {
        $regex = $this->getRegex('fly:include');
            ...
            if (preg_match_all($regex, $template, $matches, PREG_SET_ORDER)) {
                foreach ($matches as $match) {
                    ...
                    if (preg_match('/^([^\\/]+)(.*)\.'.$this->config['view_suffix'].'$/i', $file) === 1) {
                        // 支持模板引入的写法： {fly:include file="header.htm" /} 或者 {fly:include file="header" /}  或者 {fly:include file="users/header.htm" /} 或者 {fly:include file="users/header" /}
                        $file = str_replace('.'.$this->config['view_suffix'], '', $file);
                    } else if (stristr($file, '.'.$this->config['view_suffix']) && (stristr($file, '/') || stristr($file, '\\'))) {
                        // 支持绝对路径的模板引入写法： {fly:include file="/template/pc/header.htm" /}
                        $file  = '.'.$file;
                    }
                    unset($array['file']);
                    // 分析模板文件名并读取内容
     ...
     * @return void
     */
    private function parseFlyLiteral(&$content, $restore = false)
    {
        $regex = $this->getRegex($restore ? 'restoreliteral' : 'fly:literal');
        if (preg_match_all($regex, $content, $matches, PREG_SET_ORDER)) {
            if (!$restore) {
                $count = count($this->literal);
                // 替换literal标签
