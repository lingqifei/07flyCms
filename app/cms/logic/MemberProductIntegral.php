 ...
 * 会员等级升级管理=》逻辑层
 */
class MemberProductIntegral extends CmsBase
{
    /**
     * 会员等级升级列表
     * @param array $where
    ...
    {
        $list= $this->modelMemberProductIntegral->getList($where, $field, $order, $paginate)->toArray();
     ...
     * @return array
     */
    public function memberProductIntegralEdit($data = [])
    {

        $validate_result = $this->validateMemberProductIntegral->scene('edit')->check($data);
        ...
        $url = url('show');
        return $result ? [RESULT_SUCCESS, '编辑成功', $url] : [RESULT_ERROR, $this->modelMemberProductIntegral->getError()];
    }
     ...
     * @return array
     */
    public function memberProductIntegralDel($where = [])
    {

        $result = $this->modelMemberProductIntegral->deleteInfo($where,true);

        $result && action_log('删除', '删除会员等级升级，where：' . http_build_query($where));

        return $result ? [RESULT_SUCCESS, '删除成功'] : [RESULT_ERROR, $this->modelMemberProductIntegral->getError()];
    }
     ...
     * @return
     */
    public function getMemberProductIntegralInfo($where = [], $field = true)
    {

        return $this->modelMemberProductIntegral->getInfo($where, $field);
    }
