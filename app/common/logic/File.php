...
class File extends LogicBase
{
    ...
    public function pictureDown($body)
    {
        ...
        preg_match_all("/(src)=[\"|\'| ]{0,}(https:\/\/(.*)\.(gif|jpg|jpeg|bmp|png|JPEG|GIF|PNG))[\"|\'| ]{0,}/isU", $body, $img_array_https);
        $img_array = array_unique($img_array[2]);//也可以自动匹配http
        $img_array_https = array_unique($img_array_https[2]);//也可以自动匹配https

        $img_array=array_merge_recursive($img_array,$img_array_https);//合并
